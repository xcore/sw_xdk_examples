...
//                 * Touch screen *                    //
//                                                     //
//    	Simple touchscreen test and calib              //
//						       //
/////////////////////////////////////////////////////////
...
void process(chanend c_adc, chanend c_adc2, chanend toVis, int i, int v) {
    c_adc <: i;
    c_adc <: v;
    c_adc2 <: i;
    c_adc2 <: v;
...
void touch(chanend fromKeys, chanend toVis, chanend c_adc, chanend c_adc2) {
    ...
    for(int i = 0; i < BANKS; i++){
        ...
        if (v > 64) v = 63;
        if (v < 0) v = 1;
        process(c_adc, c_adc2, toVis, i, v);
    }
            ...
            if (acceptable(tmpx,tmpy)) {
                ...
                if (tmpx != x || tmpy != y) {
                    ...
                    if (y >= 0 && y < BANKS && x >= 0 && x <=64) {
                        process(c_adc, c_adc2, toVis, y, x);
                    }
                }
            }
            break;
